...
@require_GET
def index(request):
    products = Product.objects.filter(id__gt = 2)[:4][::-1];
    return render(request, 'account/authentication/index.html',{'products' : products});
...
def search(request):
    ...
    return render(request, "account/authentication/index1.html", data);
